...
-- The `Users` table stores information about individual users. The `user_id` is a non-sequential GUID.
...
-- This table should have a 1 to 1 relationship with `Users`. The goal is to separate authentication data from PII.
...
) COMMENT="Store user data in a separate table from login information. Contact information and such belongs here.";
...
-- A permission provides a string, called a permission token, that can be used programmatically allow an application's behavior based on a user's association or non-association.
-- Good development practices should prohibit almost all application behaviors that are not specifically granted. Permissions should not be granted to revoke access.
...
-- Associates  permissions with permission sets in a many-to-many relationship.
