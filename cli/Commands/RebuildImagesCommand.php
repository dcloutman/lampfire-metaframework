...
#[Command(
    name: 'rebuild-images',
    description: 'Rebuild the Docker images for the current project.',
    help: 'Rebuilds the Docker images using docker compose build --pull. This command does not start the containers.',
)]
final class RebuildImagesCommand extends AbstractCommand
{
    public function execute(Input $input, Output $output): int
    {
        try {
            ...
            $output->info('Rebuilding Docker images...');
            $resultCode = $this->runShellCommand($compose . ' build --pull', $projectRoot, $output);

            if ($resultCode !== 0) {
                throw new RuntimeException(sprintf('%s build --pull failed.', $compose));
            }

            $output->writeln();
            $output->info('Docker images rebuilt.');
            return 0;
