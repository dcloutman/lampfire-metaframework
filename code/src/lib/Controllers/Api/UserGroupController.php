...
class UserGroupController extends AbstractRestController
{
    ...
    public function get(Request $request, Response $response): Response
    {
        ...
        return $this->jsonResponse($response, ['data' => $groups]);
    }
    ...
    public function getById(Request $request, Response $response): Response
    {
        ...
        if ($group === null) {
            return $this->jsonError($response, 404, 'Not Found', 'The requested user group does not exist.');
        }

        return $this->jsonResponse($response, ['data' => $group]);
    }
    ...
    public function post(Request $request, Response $response): Response
    {
        ...
        $groupName   = $this->extractString($body, 'group_name');
        $description = $this->extractOptionalString($body, 'description');

        if ($groupName === null) {
            return $this->jsonError($response, 400, 'Bad Request', 'The group_name field is required.');
        }

        try {
            $group = $this->userGroupService->createGroup($groupName, $description);

            return $this->jsonResponse($response, ['data' => $group], 201);
